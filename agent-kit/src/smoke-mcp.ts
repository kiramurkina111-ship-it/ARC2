...
const requiredEnv = ["ARC_RPC", "ARC_CHAIN_ID", "ARC_EXPLORER", "VAULT_ADDRESS", "AGENT_PRIVATE_KEY"] as const;
const env: Record<string, string> = {};

for (const name of requiredEnv) {
  const value = process.env[name];
  if (!value) throw new Error(`${name} is required for the MCP smoke test`);
  env[name] = value;
}
