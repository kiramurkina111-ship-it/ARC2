...
server.registerTool(
  "get_vault_status",
  {
    ...
    description:
      "Read the active Paybound vault balance, spend limits, pause state, owner, agent signer, and remaining daily budget.",
    inputSchema: {}
  },
...
server.registerTool(
  "request_payment",
  {
    ...
    description:
      "Initiate a USDC payment. The vault executes it when policy passes or creates an onchain approval request when policy requires human review.",
    inputSchema: {
...
server.registerTool(
  "create_approval_request",
  {
    ...
    description:
      "Always create an onchain payment request for human review, even when the payment might fit automatic policy.",
    inputSchema: {
      recipient: z.string().describe("EVM address that should receive USDC"),
...
async function runTool<T extends object>(operation: () => Promise<T>) {
  try {
    ...
    return {
      content: [{ type: "text" as const, text: `Paybound error: ${message}` }],
      isError: true
    };
  }
}
