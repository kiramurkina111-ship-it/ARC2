...
async function runDoctor() {
    ...
    try {
        ...
        record(status.signerAuthorized ? "PASS" : "FAIL", "Agent signer", status.signerAuthorized
        ...
        record(Number(status.balanceUsdc) > 0 ? "PASS" : "WARN", "Vault balance", `${status.balanceUsdc} USDC available in the vault`);
        record(Number(status.maxSpendPerTxUsdc) > 0 && Number(status.dailyLimitUsdc) > 0 ? "PASS" : "WARN", "Spend policy", `${status.maxSpendPerTxUsdc} USDC per action; ${status.dailyLimitUsdc} USDC daily`);
        const nativeBalance = await client.provider.getBalance(status.connectedSigner);
        record(nativeBalance > 0n ? "PASS" : "WARN", "Signer gas balance", `${formatEther(nativeBalance)} native USDC on Arc Testnet`);
    }
    catch (error) {
...
function printReport() {
    ...
    if (failed === 0)
        console.log("Paybound is ready for a read-only MCP connection check.");
}
